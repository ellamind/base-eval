"""GSM-Symbolic utilities for English evaluation."""
...
def _extract_answer(continuation: str) -> str:
    ...
    where answer_regexes is NOT set (which is the case for gsm_symbolic::olmo3:n8:v2).
    Strips commas between digits and returns the last number.
    """
...
# OLMES regexes_to_ignore — inherited from GSM8K.TASK_CONFIG_DEFAULTS
# (gsm8k.py:70) via recursive merge in get_dict_with_defaults.
# gsm_symbolic::olmo3:n8:v2 does not override regexes_to_ignore,
# so it inherits: [",", "\\$", "(?s).*#### ", "\\.$"]
...
# ---------------------------------------------------------------------------
# gsm_symbolic_cot  (legacy, kept for backward compat)
# ---------------------------------------------------------------------------

def extract_gsm_answer(text: str) -> str:
    """
    Extract final answer from GSM-style response.
    OLMES-style: Try #### pattern first, fallback to last number.
    """
...
def process_results(doc: dict, results: List[str]) -> dict:
    """Legacy gsm_symbolic_cot scorer."""
    answer_text = str(doc.get("answer", "")).strip()
    gold = extract_gsm_answer(answer_text)
    gold_norm = _normalize_number(gold)

    n = len(results)
    correct = 0
    for prediction in results:
        pred = extract_gsm_answer(prediction)
        pred_norm = _normalize_number(pred)
        if gold_norm == pred_norm and gold_norm != "":
            correct += 1

    pass_at_1 = _estimate_pass_at_k(n, correct, 1)
    return {"pass_at_1": pass_at_1, "exact_match": correct / max(n, 1)}


def get_gsm_symbolic_fewshot() -> List[Dict]:
    """8-shot CoT examples for GSM-Symbolic (uses same as GSM8K, #### format)."""
    return [
        {
...
# ---------------------------------------------------------------------------
# gsm_symbolic_gen  (OLMES-aligned)
# ---------------------------------------------------------------------------

def get_gsm_symbolic_gen_fewshot() -> List[Dict]:
    """
    8-shot CoT examples for GSM-Symbolic generative task (OLMES STD:GSM8k format).
    ...
    Same fewshot source as gsm8k — both tasks use STD:GSM8k.
    """
    return [
        {
            "question": "There are 15 trees in the grove. Grove workers will plant trees in the grove today. After they are done, there will be 21 trees. How many trees did the grove workers plant today?",
...
def process_results_gen(doc: dict, results: List[str]) -> dict:
    """
    Score GSM-Symbolic generations following the exact OLMES pipeline.
    GSMSymbolic inherits from GSM8K, so the scoring is identical.
